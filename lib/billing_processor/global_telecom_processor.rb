class GlobalTelecomProcessor < BillingProcessor
  LOGIN = 'izkh'
  PASSWORD = 'FDncbv883mJ'
  ...
  def check
    if @check_url
      ...
      parse_response(er.get_basic_auth(LOGIN, PASSWORD))
    else 
      nil
    end
  end
  ...
  def pay
    if @check_url
      er = ExternalRequest.new(@check_url, true)
        response = parse_response(er.get_basic_auth(LOGIN, PASSWORD))
      else
        response = nil
      end
