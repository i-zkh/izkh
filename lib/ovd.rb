class Ovd
  ...
  def self.widget_precinct(street, house)
    street = PrecinctStreet.where("street LIKE ?", street[0..street.size-4] + '%').first
    if street
      house = PrecinctHouse.where("house = ? and precinct_street_id = ?", house.mb_chars.downcase, street.id).first
      precinct = Precinct.find(house.precinct_id) if house
    end
    precinct
  end
